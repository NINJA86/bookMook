...
import { ICommentPopulated } from '@/lib/type';

export function FeaturedCommentsCarousel() {
  const [featuredComments, setFeaturedComment] = useState<ICommentPopulated[]>(
    [],
  );
  useEffect(() => {
    const fetchFeaturedComments = async () => {
      const commentData = await bookService.getFeatruedComments();
      console.log(commentData);
      setFeaturedComment(commentData);
    };
    fetchFeaturedComments();
    console.log(`featuredComments`);
  }, []);
  return (
    <section className="mx-auto max-w-7xl space-y-8 px-6 py-12">
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <h2 className="font-display text-4xl font-semibold tracking-tight">
            Readers who keep the shelves moving
          </h2>
        </div>
        <p className="max-w-2xl text-sm text-muted-foreground">
          A flowing wall of reader reactions inspired by modern testimonial
          sections, with local avatars and continuously moving feedback cards.
        </p>
      </div>
      ...
      >
        {featuredComments.map((review: ICommentPopulated) => (
          <SwiperSlide key={`${review._id}-featured`} className="h-auto">
